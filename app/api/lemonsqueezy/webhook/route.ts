...
export async function POST(request: Request) {
  try {
    ...
    // Handle subscription_created and subscription_updated events
    if (eventName === 'subscription_created' || eventName === 'subscription_updated') {
      const subscription = event.data;
      const attributes = subscription.attributes;

      // Extract custom data from event.meta (correct location!)
      ...
      console.log('[LEMONSQUEEZY] Processing subscription event:', {
        event: eventName,
        userId,
        tier,
